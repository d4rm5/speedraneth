...
const Events: NextPage = () => {
  ...
  const { data: tokenApprovedEvents, isLoading: isTokenApprovedEventLoading } = useScaffoldEventHistory({
    ...
    <>
      <div className="flex items-center flex-col flex-grow pt-10">
        ...
        {isTokenApprovedEventLoading ? (
          ...
          <div className="mt-8">
            <div className="text-center mb-4">
              <span className="block text-2xl font-bold">Token Approved Events</span>
            </div>
            <div className="overflow-x-auto shadow-lg">
              <table className="table table-zebra w-full">
                <thead>
                  <tr>
                    ...
                    <th className="bg-primary">Tokens Approved</th>
                  </tr>
                </thead>
                <tbody>
                  {!tokenApprovedEvents || tokenApprovedEvents.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="text-center">
                        No events found
                      </td>
                    ...
                    tokenApprovedEvents?.map((event, index) => {
                      return (
                        <tr key={index}>
                          <td className="text-center">
                            <Address address={event.args.spender} />
                          </td>
                          <td>{parseFloat(event.args.amount)}</td>
                        </tr>
                      );
                    })
                  )}
